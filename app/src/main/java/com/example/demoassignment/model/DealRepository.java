...
public class DealRepository {
    ...
    private MutableLiveData<List<Deal>> dealLiveData = new MutableLiveData<>();
    ...
    public DealRepository() {
        ...
        dealLiveData.setValue(new ArrayList<>());
        loadDeals();
    }
    ...
    public void loadDeals() {
        ...
        apiService.getDeals( TOTAL_PAGES, currentPage, "id,created_at,created_at_in_millis,image_medium,comments_count,store{name}").enqueue(new Callback<ApiResponse>() {
            @Override
                ...
                if (response.body() != null) {
                    ...
                    if (apiResponse != null) {
                        ...
                        if (deals != null && deals.size() > 0) {
                            currentPage++;
                            List<Deal> currentData = dealLiveData.getValue();
                            currentData.addAll(deals);
                            dealLiveData.setValue(currentData);
                        } else {
                            isLastPage = true;
                        }
